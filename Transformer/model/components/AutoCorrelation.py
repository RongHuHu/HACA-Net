...
class AutoCorrelation(nn.Module):
    ...
    def time_delay_agg_training(self, values, corr):
        ...
        This is for the training phase.训练阶段
        看懂这个函数决定你是否能把自己的想法移植过来
        
        corr_shape = (32,8,96,64) batch-head-length-dimension
        """   
        head = values.shape[1]
...
class AutoCorrelationLayer(nn.Module):
    ...
    def forward(self, queries, keys, values, attn_mask):
        ...
        # tsam_q = ts_channel_block(7)
        # tsam_k = ts_channel_block(7)
        # tsam_v = ts_channel_block(7)
        # queries = tsam_q(queries) 
        # keys = tsam_k(keys)
        # values = tsam_v(values)
