...
class GMM2D(td.Distribution):
    ...
    def __init__(self, log_pis, mus, log_sigmas, corrs):
        super(GMM2D,
              self).__init__(batch_shape=log_pis.shape[0],
        ...
        self.L = torch.stack([torch.stack([self.sigmas[...,
                                                       0],
                                           torch.zeros_like(self.log_pis)],
                                          dim=-1),
                              torch.stack([self.sigmas[...,
                                                       1] * self.corrs,
                                           self.sigmas[...,
                                                       1] * torch.sqrt(self.one_minus_rho2)],
                                          dim=-1)],
                             dim=-2)
    ...
    def from_log_pis_mus_cov_mats(cls, log_pis, mus, cov_mats):
        ...
        sigmas = torch.stack(
            [torch.sqrt(sigma_1), torch.sqrt(sigma_2)], dim=-1)
    ...
    def reg_los(self, value):
        ...
        l_reg = torch.sqrt(torch.sum((dx**2), dim=-1) + 1e-8)
        # print("l_reg size : ",l_reg.size())
        return l_reg
    ...
    def get_for_node_at_time(self, n, t):
        return self.__class__(self.log_pis[:, n:n +
                                           1, t:t +
                                           1], self.mus[:, n:n +
                                                        1, t:t +
                                                        1], self.log_sigmas[:, n:n +
                                                                            1, t:t +
                                                                            1], self.corrs[:, n:n +
                                                                                           1, t:t +
                                                                                           1])
    ...
    def reshape_to_components(self, tensor):
        if len(tensor.shape) == 5:
            return tensor
        return torch.reshape(tensor, list(
            tensor.shape[:-1]) + [self.components, self.dimensions])
