...
class Encoder(nn.Module):
    def __init__(
        ...
        e_layers1=[3,2,1]
        distil=True
        dow_inp_lens = list(range(len(e_layers1))) # [0,1,2,...] you can customize here        
        dow_encoders = [
            InformerEncoder(
                [
                    EncoderLayer(
        ...
        e_layers2=[1,2,3]
        distil=True
        ups_inp_lens = list(range(len(e_layers2))) # [0,1,2,...] you can customize here        
        ups_encoders = [
            InformerEncoder(
                [
                    EncoderLayer(
                ...
                [
                    UpSampleLayer(256) for l in range(el-1)
                ] if distil else None,
                norm_layer=torch.nn.LayerNorm(256)
            )for el in e_layers2]
        
        self.encoder = EncoderStack(dow_encoders, ups_encoders, dow_inp_lens, ups_inp_lens)
        # self.initial_h = nn.Linear(ninp, in_dim//2)
        # self.gru = nn.GRU(input_size=ninp, 
    ...
    def forward(self, src, robot, src_mask, src_key_padding_mask, robot_padding_mask, robot_src_mask):
        ...
        if robot != None:
            ...
            robot = robot.permute(0,2,1)
        # robot_present, robot_future = robot[:, [0], :], robot[:, 1:, :]
        # robot = (robot_present.expand(robot_present.shape[0], 6, robot_present.shape[2])).permute(1, 0, 2)
        # h = self.initial_h(robot)
        # robot, _ = self.gru(robot_future, h)
...
class Decoder(nn.Module):
    def __init__(
        ...
        self.decoder = InformerDecoder(
            [
                DecoderLayer(
                    AttentionLayer(ProbAttention(True, factor=5, attention_dropout=0.1, output_attention=False), 
...
class UpSampleLayer(nn.Module):
    def __init__(self, c_in):
        ...
        self.conv_transpose = nn.ConvTranspose1d(c_in, c_in, kernel_size=4, stride=2, padding=1)
        self.norm = nn.BatchNorm1d(c_in)
        self.activation = nn.ELU()

    def forward(self, x):
        #print('x20',x.size())
        x = self.conv_transpose(x.transpose(1,2))    #permute(0, 2, 1))
        #print('x21',x.size())
        x = self.norm(x)
        x = self.activation(x)
        x = x.transpose(1,2)
...
class EncoderStack(nn.Module):
    def __init__(self, dow_encoders, ups_encoders, dow_inp_lens, ups_inp_lens):
        super(EncoderStack, self).__init__()
        self.dow_encoders = nn.ModuleList(dow_encoders)
        self.ups_encoders = nn.ModuleList(ups_encoders)
        self.dow_inp_lens = dow_inp_lens
        self.ups_inp_lens = ups_inp_lens
        self.tcs_layer = Times_Channel_Squeeze()

    def forward(self, x, attn_mask=None):
        # x [B, L, D]
        ups_x_stack = []; attns = []
        for ups_i_len, ups_encoder in zip(self.ups_inp_lens, self.ups_encoders):
            ups_inp_len = x.shape[1]//(2**ups_i_len)
            ups_x_s, attn = ups_encoder(x[:, -ups_inp_len:, :]) #(x[:, -ups_inp_len:, :])
            #print('ups_x_s',ups_x_s.size())
            ups_x_stack.append(ups_x_s) ; attns.append(attn)   
        ups_x_stack = torch.cat(ups_x_stack, -2)
        #print('ups_x_stack',ups_x_stack.size())
        x = self.tcs_layer(ups_x_stack)
        #print('x_fusion1',x.size())

        dow_x_stack = []
        for dow_i_len, dow_encoder in zip(self.dow_inp_lens, self.dow_encoders):
            dow_inp_len = x.shape[1]//(2**dow_i_len)
            dow_x_s, attn = dow_encoder(x[:, -dow_inp_len:, :]) #(x[:, -dow_inp_len:, :])
            #print('dow_x_s',dow_x_s.size())
            dow_x_stack.append(dow_x_s) ; attns.append(attn)  
        dow_x_stack = torch.cat(dow_x_stack, -2)
        #print('dow_x_stack',dow_x_stack.size())
