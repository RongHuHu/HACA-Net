...
class MultimodalGenerativeCVAE(object):
    ...
    def obtain_encoded_tensors(
        ...
        memory_padding_mask = generate_mask(memory).to(self.device)
        memory_src_mask = generate_square_subsequent_mask(memory.size()[-2], self.device)
        # avg_pool = nn.AdaptiveAvgPool1d(1)
        # memory_src_mask = avg_pool(memory_src_mask)
        # memory_src_mask = memory_src_mask.permute(1,0)       
    ...
    def encode_total_edge_influence(self, encoded_edges, node_history_encoder, batch_size):
        if len(encoded_edges) == 0:
                ...
                # print('combined_edges',combined_edges.size())
                avg_pool = nn.AdaptiveAvgPool1d(1)
    ...
    def GMM_dec(
        ...
        for _ in range(prediction_horizon):
            ph=ph+1
            new_state = a_t[:, [-1], :]
            tgt_mask = generate_square_subsequent_mask(new_state.size()[-2], self.device)
            h = transformer_decoder(
                ph,
                tgt=new_state,
                memory=memory,
