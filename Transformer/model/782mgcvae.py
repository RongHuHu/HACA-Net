...
class MultimodalGenerativeCVAE(object):
    ...
    def GMM_dec(
        ...
        mlp_g2 = self.node_modules[self.node_type + "/GMM/MLP2"]
        history_input = init_pos.view(batch_size, 1, pred_state)
        ...
        memory_key_padding_mask=memory_key_padding_mask.repeat(num_samples * num_components, 1, 1)
        memory_mask=memory_mask.repeat(num_samples * num_components, 1, 1)
        memory=memory.repeat(num_samples * num_components, 1, 1)
        for _ in range(prediction_horizon):
            ph=ph+1
            tgt_mask = generate_square_subsequent_mask(ph, self.device)
            if ph > 1:
                log_pi_t, mu_t, log_sigma_t, corr_t = self.project_to_GMM_params(mlp_g(a_t[:, [-1], :]))
                # print('log_pi_t',log_pi_t.size()) 
                # print('mu_t',mu_t.size()) 
                # print('log_sigma_t',log_sigma_t.size()) 
                # print('corr_t',corr_t.size())            
        ...
        # print('log_pis',log_pis.size()) 
        # print('mus',mus.size()) 
        # print('log_sigmas',log_sigmas.size()) 
        # print('corrs',corrs.size())
    ...
    def train_decoder(
        ...
        if autoregressive: 
            ...
            y_dist, sampled_future = self.GMM_dec(
                mode,
                inputs_st,
                memory,
                memory_mask,
    ...
    def tc_lgm_logits(feat, num_classes=25, labels=None, alpha=0.1, lambda_=0.01, means=None):
        N= feat.size()[0]
    ...
    def train_loss(
        ...
        if self.hyperparams["autoregressive"]:
            ...
            y_dist, history_pred = self.train_decoder(
                mode,
                inputs_st,
                self.hyperparams["autoregressive"],
                memory,
            ...
            # log_p_yt_reg = y_dist.reg_los(labels_st)
            # log_p_y_reg = torch.sum(log_p_yt_reg, dim=2) # 1, 128
            # log_p_y_mean_reg = torch.mean(log_p_y_reg, dim=0) # 128
            # log_likelihood_reg = torch.mean(log_p_y_mean_reg)
            # reg_loss = log_likelihood_reg
            
            lambda_=0
            alpha=1.0           
            loss = lambda_ * reg_loss + alpha * nll

            # lgmloss = LGMLoss(2, history_pred.size()[-1], alpha=1.0, lambda_=1.0).to(self.device)
            # _, reg_loss, _ = lgmloss(history_pred, labels)
            # _, reg_loss, _ = tc_lgm_logits(history_pred, 
            #                                 labels=labels,
            #                                 alpha=1.0,
            #                                 lambda_=1.0,
            #                                 means=lgmloss.means)
    ...
    def eval_loss(
        ...
        if self.hyperparams["autoregressive"]:
            ...
            _, history_pred= self.test_decoder(
                mode,
                inputs_st,
                self.hyperparams["autoregressive"],
                memory,
    ...
    def predict(
        ...
        if self.hyperparams["autoregressive"]:
            ...
            _, our_sampled_future = self.test_decoder(
                mode,
                inputs_st,
                self.hyperparams["autoregressive"],
                memory,
