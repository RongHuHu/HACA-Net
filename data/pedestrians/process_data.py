...
sys.path.append("../../Transformer") #trajectron
...
for desired_source in ['eth', 'hotel', 'univ', 'zara1', 'zara2']:
    for data_class in ['train', 'val', 'test']:
        ...
        scenes = []
        data_dict_path = os.path.join('../../experiments/processed_data', '_map_'.join([desired_source, data_class]) + '.pkl')
        ...
        for subdir, dirs, files in os.walk(os.path.join('raw', desired_source, data_class)):
            for file in files:
                if file.endswith('.txt'):
                    ...
                    scene = Scene(timesteps=max_timesteps+1, dt=dt, name=desired_source + "_map_" + data_class, aug_func=augment if data_class == 'train' else None)
