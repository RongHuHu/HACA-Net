...
class MultimodalGenerativeCVAE(object):
    ...
    def encode_node_history(self, mode, node_hist, first_history_indices):
        ...
        return outputs[torch.arange(first_history_indices.shape[0]).type(torch.long), last_index_per_sequence.type(torch.long)]
    ...
    def encode_edge(self,
        ...
        last_index_per_sequence = -(first_history_indices + 1)
        ret = outputs[torch.arange(last_index_per_sequence.shape[0]).type(torch.long), last_index_per_sequence.type(torch.long)]
        if self.hyperparams['dynamic_edges'] == 'yes':
