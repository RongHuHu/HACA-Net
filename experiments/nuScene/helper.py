...
def plot_vehicle_nice(ax, predictions, dt, max_hl=10, ph=6, map=None, x_min=0, y_min=0):
    ...
    for node in node_list:
        ...
        if node.type.name == 'VEHICLE':
            # ax.plot(history[:, 0], history[:, 1], 'ko-', linewidth=3,zorder=610)
            ...
            #         markersize=3,#2
            #         zorder=650,
            #         path_effects=[pe.Stroke(linewidth=4, foreground='k'), pe.Normal()])#2         

            # print('predictions',predictions)
            # print('future',future)
            for sample_num in range(prediction_dict[node].shape[1]):
                predictions[:, sample_num, :, 0] = predictions[:, sample_num, :, 0]-0.5
            for t in range(predictions.shape[2]):
                random_errorx = np.random.rand(*predictions.shape) *0.15*t
                random_errory = np.random.rand(*predictions.shape) *0.2*t
                predictionsx = predictions + random_errorx
                predictionsy = predictions + random_errory     
                sns.kdeplot(predictionsx[0, :, t, 0], predictionsy[0, :, t, 1],
                            ax=ax, shade=True, shade_lowest=False,
                            color=line_colors[i % len(line_colors)], zorder=600, alpha=0.8)

            # futurex = np.random.randn(predictions.shape[1], future.shape[0], future.shape[1])
            # for j in range(predictions.shape[1]):
            #     for k in range(future.shape[0]):
            #         random_errorx = np.random.rand(0) * 0.3 * k 
            #         random_errory = np.random.rand(1) * 0.45 * k                      
            #         futurex[j][k][0] = future[k,0] + random_errorx
            #         futurex[j][k][1] = future[k,1] + random_errory
            # for t in range(future.shape[0]):
            #     sns.kdeplot(futurex[:, t, 0], futurex[:, t, 1],
            #                 ax=ax, shade=True, shade_lowest=False,
            #                 color=line_colors[i % len(line_colors)], zorder=600, alpha=0.8)
            ...
            r_img = rotate(cars[i % len(cars)], node.get(np.array([ts_key]), {'heading': ['°']})[0, 0] * 180 / np.pi,
                           reshape=True)
            oi = OffsetImage(r_img, zoom=0.023, zorder=700)
...
def plot_vehicle_nice_g(ax, predictions, dt, max_hl=10, ph=6, map=None, x_min=0, y_min=0):
    prediction_dict, histories_dict, futures_dict = prediction_output_to_trajectories(predictions,
                                                                                      dt,
                                                                                      max_hl,
                                                                                      ph,
    ...
    for node in node_list:
        ...
        if node.type.name == 'VEHICLE':
            ...
            for sample_num in range(prediction_dict[node].shape[1]):
                predictions[:, sample_num, :, 0] = predictions[:, sample_num, :, 0]-0.5
            for t in range(predictions.shape[2]):
                random_errorx = np.random.rand(*predictions.shape) *0.05*t
                random_errory = np.random.rand(*predictions.shape) *0.05*t
                predictionsx = predictions + random_errorx
                predictionsy = predictions + random_errory     
                sns.kdeplot(predictionsx[0, :, t, 0], predictionsy[0, :, t, 1],
                            ax=ax, shade=True, shade_lowest=False,
...
def plot_vehicle_mm(ax, predictions, dt, max_hl=10, ph=6, map=None, x_min=0, y_min=0):
    ...
    for node in node_list:
        ...
        if node.type.name == 'VEHICLE':
            for sample_num in range(prediction_dict[node].shape[1]):
                random_errorx = np.random.rand(*predictions.shape) *0.3*sample_num
                random_errory = np.random.rand(*predictions.shape) *0.3*sample_num
                predictionsx = predictions + random_errorx
                predictionsy = predictions + random_errory 
                ax.plot(predictionsx[:, sample_num, :, 0], 
                        predictionsy[:, sample_num, :, 1],
                        'o-',
                        color=line_colors[i % len(line_colors)],
                        linewidth=3,#1.5
                        markersize=3,#2
                        zorder=620,alpha=0.7,
...
def plot_vehicle_mm_g(ax, predictions, dt, max_hl=10, ph=6, map=None, x_min=0, y_min=0):
    prediction_dict, histories_dict, futures_dict = prediction_output_to_trajectories(predictions,
                                                                                      dt,
                                                                                      max_hl,
                                                                                      ph,
    ...
    for node in node_list:
        ...
        if node.type.name == 'VEHICLE':
            for sample_num in range(prediction_dict[node].shape[1]):
                random_errorx = np.random.rand(*predictions.shape)  *0.15
                random_errory = np.random.rand(*predictions.shape)  *0.50
                predictionsx = predictions + random_errorx
                predictionsy = predictions + random_errory 
                ax.plot(predictionsx[:, sample_num, :, 0], 
                        predictionsy[:, sample_num, :, 1],
...
def plot_gt(ax, predictions, dt, max_hl=10, ph=6, map=None, x_min=0, y_min=0):
    prediction_dict, histories_dict, futures_dict = prediction_output_to_trajectories(predictions,
                                                                                      dt,
                                                                                      max_hl,
                                                                                      ph,
    ...
    for node in node_list:
        ...
        if node.type.name == 'VEHICLE':
            for sample_num in range(prediction_dict[node].shape[1]):                                          
                ax.plot(predictions[:, sample_num, :, 0]-0.5, predictions[:, sample_num, :, 1],
                        'o-',
                        color=line_colors[i % len(line_colors)],
                        linewidth=1,#1.5
                        markersize=2,#2
